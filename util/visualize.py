...
def plot_bounding_boxes_on_image_cv2(
    ...
    text_box_color: Tuple[int, int, int] = (255, 255, 255),
    ...
    :param font_scale: scale factor to set font size, defaults to 1.0 设置字体大小
    :param box_thick: scale factor to set box border weight, defaults to 3 设置边的粗细
    :param fill_alpha: alpha to filling the area in the bounding box, defaults to 0.2 填充透明度
    :param text_box_color: background color of the text box, defaults to (255, 255, 255) 文本框背景颜色
    :param text_font_color: text color, will be set automatically if not given, defaults to None 文本颜色
    :param text_alpha: alpha to filling the area in the text box, defaults to 0.5 文本框透明度
    """
    if len(labels) == 0:
        return image
...
def plot_red_bounding_boxes_on_image_cv2(
        image: np.ndarray,
        boxes: Union[np.ndarray, List[float]],
        labels: Union[np.ndarray, List[int]],
        scores: Union[np.ndarray, List[float]] = None,
    ...
    # 固定为红色
    red = (0, 0, 255)  # 红色在BGR格式下是(0, 0, 255)
    light_red = (180, 180, 255)  # 浅红色
    dark_red = (0, 0, 150)  # 深红色

    colors = [red] * len(boxes)
    light_colors = [light_red] * len(boxes)
    dark_colors = [dark_red] * len(boxes)
