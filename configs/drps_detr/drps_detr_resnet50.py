...
from models.bricks.drps_transformer import (
    DRPSTransformer,
    DRPSTransformerDecoder,
    DRPSTransformerDecoderLayer,
    DRPSTransformerEncoder,
    DRPSTransformerEncoderLayer,
)
from models.bricks.set_criterion import HybridSetCriterion
from models.detectors.drps_detr import SalienceCriterion, DRPSDETR
...
transformer = DRPSTransformer(
    encoder=DRPSTransformerEncoder(
        encoder_layer=DRPSTransformerEncoderLayer(
            embed_dim=embed_dim,
            n_heads=num_heads,
            dropout=0.0,
    ...
    neck=None,
    decoder=DRPSTransformerDecoder(
        decoder_layer=DRPSTransformerDecoderLayer(
            embed_dim=embed_dim,
            n_heads=num_heads,
            dropout=0.0,
            activation=nn.ReLU(inplace=True),
...
weight_dict.update({k + "_o2m": v for k, v in weight_dict.items()})
