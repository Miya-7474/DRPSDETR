...
class SalienceCriterion(nn.Module):
    ...
    def get_pixel_coordinate(self, feature_shape, stride, device):
        ...
        coord_y, coord_x = torch.meshgrid(
            #生成特征图每一行的中心点坐标（从0.5到height-0.5），乘以stride后映射到原图坐标。
...
class SalienceCriterionLayer(nn.Module):
    def __init__(
        self,
        limit_range: Tuple = ((-1, 64), (64, 128), (128, 256), (256, 99999)),
        noise_scale: float = 0.0,
    ...
    def forward(self, foreground_mask, targets, feature_strides, image_sizes):
        ...
        for level_idx, (mask, feature_stride) in enumerate(zip(foreground_mask, feature_strides)):
            ...
            masks_per_level = torch.stack(masks_per_level)# [batch, h*w]
            mask_targets.append(masks_per_level)

        #mask_targets = torch.cat(mask_targets, dim=1)# [batch, sum(h*w),]

        foreground_mask_flatten = []
        #loss_per_weight = []
        for mask in foreground_mask:
            mask = mask.flatten(-2).squeeze(1)
            foreground_mask_flatten.append(mask)
            #loss_per_weight.append(mask.shape[1])
        # foreground_mask = torch.cat([e.flatten(-2) for e in foreground_mask], -1)
        # foreground_mask = foreground_mask.squeeze(1)

        salience_loss= {}
        for i in range(len(mask_targets)):
            num_pos = torch.sum(mask_targets[i] > 0.5 * self.noise_scale).clamp_(min=1)
            loss = (
                sigmoid_focal_loss(
                    foreground_mask_flatten[i],
                    mask_targets[i],
                    num_pos,
                    alpha=self.alpha,
                    gamma=self.gamma,
                ) * foreground_mask_flatten[1].shape[1]
            )
            dict_loss = {f"loss_salience_{i}": loss}
            salience_loss.update(dict_loss)
...
class DRPSDETR(DNDETRDetector):
    ...
    def forward(self, images: List[Tensor], targets: List[Dict] = None):
        ...
        if self.training:
            # pre o2m loss
            output_o2m = {"pred_logits": outputs_class_o2m[-1], "pred_boxes": outputs_coord_o2m[-1]}
            if self.aux_loss:
                output_o2m["aux_outputs"] = self._set_aux_loss(outputs_class_o2m, outputs_coord_o2m)
            output_o2m["enc_outputs"] = {"pred_logits": enc_class_o2m, "pred_boxes": enc_coord_o2m}
            ...
            for t in multi_targets:
                t["boxes"] = t["boxes"].repeat(6, 1)
                t["labels"] = t["labels"].repeat(6)
            o2m_losses = self.criterion(output_o2m, multi_targets)
            loss_dict.update({k + "_o2m": v for k, v in o2m_losses.items()})
